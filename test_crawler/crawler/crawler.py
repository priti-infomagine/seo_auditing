...
Web Crawler
===========
Main crawler implementation using Playwright.
"""
...
class WebCrawler:
    ...
    async def crawl(self, url: str) -> CrawlerResponse:
        ...
        Raises:
            RuntimeError: If browser not initialized or crawl fails
        """
        if not self.browser:
        ...
        try:
            ...
            performance_metrics = await PerformanceCollector.collect_from_page(page)
            
            # Collect SSL info
            ssl_info = await SecurityCollector.collect_ssl_info(page)
            
            # Collect robots and sitemap
            robots_collector = RobotsCollector(normalized_url)
            robots_sitemap_data = await robots_collector.collect_all(page)
