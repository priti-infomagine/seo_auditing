...
class CrawlDataParser:
    ...
    def parse_html(self, html: str, url: str = "", crawler_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Parse HTML content directly and extract SEO-relevant data.
        ...
        data = {
            "basic": {
                ...
                "title_length": self._get_title_length(soup),
                ...
                "doctype": self._get_doctype(html),
            },

            "http": {
                "status_code": http_data.get("status_code", 200),
                "response_time": http_data.get("response_time", 0),
            ...
            "url": {
                "url_structure": self._get_url_structure(url),
            ...
            "headings": {
                ...
                "heading_stats": self._get_heading_stats(soup),
            },

            "content": {
                "word_count": self._get_word_count(soup),
                "reading_time": self._get_reading_time(soup),
                "paragraph_count": self._get_paragraph_count(soup),
                "text_html_ratio": self._get_text_html_ratio(html, soup),
                "content_hash": self._get_content_hash(soup),
            },
            ...
            "media": {
                "videos": self._get_video_count(soup),
                "iframes": self._get_iframe_count(soup),
                "tables": self._get_table_count(soup),
                "lists": self._get_lists(soup),
                "forms": self._get_form_count(soup),
                "buttons": self._get_button_count(soup),
            },
    ...
    def analyze_seo_score(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            SEO analysis results with on-page, off-page, technical, geo, and local scores
        """
        ...
        # Title checks - support both old and new data structures
        ...
        # Meta description checks - support both old and new data structures
        ...
        # Heading checks - support both old and new data structures
        headings = page_data.get('headings', {})
        headings_data = headings.get('headings', headings)  # Support nested structure
        ...
        # Image alt text checks - support both old and new data structures
        ...
        # Word count check - support both old and new data structures
        ...
        # URL structure check - support both old and new data structures
        ...
        # Internal linking - support both old and new data structures
        ...
        # Lazy loading - support both old and new data structures
        ...
        # SSL Certificate - support both old and new data structures
        ...
        # Mobile friendly - support both old and new data structures
        ...
        # Canonical URL check - support both old and new data structures
        ...
        # Language check - support both old and new data structures
        ...
        # Open Graph check - support both old and new data structures
        ...
        # Schema markup check - support both old and new data structures
        ...
        # Text/HTML ratio - support both old and new data structures
        content_data = page_data.get('content', {})
        ...
        return {
            ...
            "breakdown": {
                ...
                "geo_seo": {
                    "score": geo_score,
om/                    "grade": self._get_grade(geo_score),
                    "issues": geo_issues,
    ...
    def generate_report_by_domain(self, domain: str) -> Optional[Dict[str, Any]]:
        ...
        report = {
            ...
            "technical": {
                ...
                "ssl_certificate": page_data.get('ssl_certificate', False),
            ...
            "geo_local": {
                ...
                "phone_numbers": page_data.get('phone_numbers', []),
                "business_hours": page_data.get('business_hours', {}),
                "google_my_business": page_data.get('google_my_business', {}),
                "local_business_schema": page_data.get('local_business_schema', {}),
                "social_media_links": page_data.get('social_media_links', [])
            }
        }
    ...
    def save_parsed_data(self, report: Dict[str, Any], domain: str) -> Path:
        ...
        # Find next test number by checking existing files
        existing_files = list(parsed_dir.glob(f"{domain}_test_*.json"))
        if existing_files:
            # Extract test numbers and find the highest
            test_numbers = []
            for f in existing_files:
                try:
                    # Extract number between '_test_' and '_timestamp'
                    filename = f.stem
                    test_part = filename.split('_test_')[1]
                    # Get only the test number (before the timestamp)
        ...
        # Generate filename with timestamp and test number
    ...
    def print_report(self, report: Dict[str, Any]) -> None:
        ...
        if geo_local:
            ...
            if address.get('found'):
                ...
                if address.get('structured_data'):
                    print(f"  Structured Address: {address['structured_data'].get('type', 'N/A')}")
            ...
            local_schema = geo_local.get('local_business_schema', {})
            if local_schema.get('found'):
                print(f"  Local Business Schema: {local_schema.get('type', 'N/A')}")
...
def main():
    ...
    # Check if URL/domain provided as argument
    if len(sys.argv) >= 2:
        ...
        if crawl_dir.exists():
            ...
            if domains:
                ...
                choice = input("\nEnter domain number, domain name, or URL: ").strip()
                input_value = choice
            else:
                input_value = input("\nEnter URL or domain to analyze: ").strip()
        else:
            input_value = input("\nEnter URL or domain to analyze: ").strip()
    ...
    try:
        ...
        # Check if input is a domain name (no protocol, no slashes)
        if '.' in input_value and '://' not in input_value and '/' not in input_value:
            # Treat as domain name
            print(f"\nParsing domain: {input_value}")
            report = parser.generate_report_by_domain(input_value)
            domain = input_value
        else:
            # Treat as URL
            print(f"\nParsing URL: {input_value}")
            report = parser.generate_report(input_value)
            # Extract domain from URL
            from urllib.parse import urlparse
            domain = urlparse(input_value).netloc.replace('www.', '')

        if report:
            # Save parsed data to file
            saved_path = parser.save_parsed_data(report, domain)
            print(f"\n✅ Parsed data saved to: {saved_path}")
            
            # Print report
            parser.print_report(report)
        else:
            print(f"No crawl data found for: {input_value}")
            sys.exit(1)
