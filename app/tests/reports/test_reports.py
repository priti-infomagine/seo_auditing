...
def test_crawl_job_model_has_audit_id_column():
    """The CrawlJob model must expose a audit_id column so that CrawlResponse.audit_id
    can be used to look up the CrawlJob later (the crawl endpoint sets audit_id, and
    public report endpoints accept it).
    """
    from app.modules.crawler.models.crawl_jobs import CrawlJob
    assert hasattr(CrawlJob, "audit_id"), "CrawlJob model is missing audit_id column"


def test_crawl_job_repository_has_audit_id_lookup():
    """CrawlJobRepository must provide get_by_audit_id and get_by_id_or_audit_id."""
    from app.modules.crawler.repositories.crawl_job_repository import CrawlJobRepository
    assert hasattr(CrawlJobRepository, "get_by_audit_id")
    assert hasattr(CrawlJobRepository, "get_by_id_or_audit_id")


def test_build_audit_report_uses_audit_id_fallback():
    """build_audit_report must resolve the crawl job via get_by_id_or_audit_id
    so that a audit_id (not just audit_id) can be used to fetch the report.

    Asserted at the source level (no DB fixture needed).
    """
    import inspect
    src = inspect.getsource(build_audit_report)
    assert "get_by_id_or_audit_id" in src
    assert "resolved_audit_id" in src
