...
class BatchParserService:
    ...
    - pass crawl result to ParserOrchestrator
    - attach source crawl metadata
    - persist parsed output

    ParserOrchestrator knows nothing about filesystem paths.
    """
    ...
    def parse_website(self, website: str) -> dict[str, Any]:
        ...
        parsed_page = self.parser.parse(html=html, url=url)

        parsed_output = parsed_page.model_dump(mode="json")
    ...
    def _get_latest_crawl_file(self, domain: str) -> Path | None:
        candidates = [domain]
